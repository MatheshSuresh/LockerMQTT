...
const Dashboard = () => {
    ...
    const getInfo = async () => {
        try {
            ...
            var user = await axios.get(`http://3.239.93.89:4001/user/check`).then((res) => { return res.data })
    ...
    const handleUnlock = async (values) => {
        await axios.post("http://3.239.93.89:4001/locker/unlock", values)
        var datanew = await axios.post("http://3.239.93.89:4001/locker/unlockupdate", values)
        if (datanew !== null) {
            setLock("unlock");
            console.log(lock);
            getInfo();
    ...
    const addlocker = async () => {
        ...
        var datanew = await axios.post("http://3.239.93.89:4001/locker/updateuser", data)
        console.log(datanew)
        if (datanew !== null) {
            setModal(false)
